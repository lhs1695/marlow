...
- 「请调查 INC-1001 并关单」——查工单、搜钉死版本的 Grafana 手册、写评论并关单。
- 「读取 INC-1005 的资产配置」——下游超时，Run 按故障收口，不装成已经改好了。
- 「工单 CHG-2004 申请给 emp-007 加 Grafana Editor」——一线只能提出变更，真正改权限要管理员批。
...
缺聊天 Key 时 `--real` 自动 Fake。p50 / p95 / token：`uv run python -m marlow.live`（case 2 × 10），报告在 `evals/live/reports/`（gitignore），不进 CI、不进默认 `pytest`。字段与环境变量见 `evals/live/README.md`。
